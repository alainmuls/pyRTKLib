...
def rise_set_times(prn: str, df: pd.DataFrame, nomint_multi: int, logger: logging.Logger) -> Tuple[list, list]:
    """
    ...
    prn_loc = df.loc[df['PRN'] == prn].index
    # logger.info('{func:s}: Data for {prn:s} are at indices\n{idx!s}'.format(prn=prn, idx=prn_loc, func=cFuncName))

    # create a df_prn only using these rows
    df_prn = df.iloc[prn_loc]
    ...
    logger.info('{func:s}:    nominal time interval for {prn:s} = {tint:f}'.format(prn=prn, tint=nominal_interval, func=cFuncName))
    ...
    logger.info('{func:s}:    {prn:s} rises at:\n{arcst!s}'.format(prn=prn, arcst=df_prn.loc[idx_arc_start][['DATE_TIME', 'PRN', 'gap']], func=cFuncName))

    idx_arc_end = df_prn[df_prn['gap'].shift(-1) > nomint_multi * nominal_interval].index.tolist() + [df_prn.index[-1]]
    logger.info('{func:s}:    {prn:s} sets at:\n{arcend!s}'.format(prn=prn, arcend=df_prn.loc[idx_arc_end][['DATE_TIME', 'PRN', 'gap']], func=cFuncName))
    ...
    df.loc[df_prn.index, 'gap'] = df_prn['gap']
